...
class Pitisan extends CI_Controller {
	...
	public function _remap($method, $params = array())
	{
		switch ( $method ) {
			case 'hmvc':
			$this->hmvc(isset($params[0]) ? $params[0] : Null);
				break;
			case 'hmvc_controller':
				$this->hmvc_controller(
					isset($params[0]) ? $params[0] : Null,
					isset($params[1]) ? $params[1] : Null,
					isset($params[2]) ? $params[2] : 'CI_Controller'
				);
				break;	
			case 'hmvc_model':
				$this->hmvc_model(
	...
	protected function _controller_creator($name, $extendsName)
	{
		...
		$data = "<?php\n\nclass ".ucfirst($name)." extends ".$extendsName." {\n";
		$data .= "\n	public function __construct()\n";
		$data .= "	{\n";
	...
	{
		$name = explode('/', $name)[count(explode('/', $name))-1]; // Find the end of array
		$data = "<?php\n\nclass ".ucfirst($name)."_model extends ".$extendsName." {\n";
		if ( isset($table) ) {
	...
	public function hmvc($name = Null)
	{
		$folder = APPPATH.'modules'.'/';
		$folder .= $name;
		mkdir( $folder );
		mkdir( $folder .'/'. 'controllers' );
		mkdir( $folder .'/'. 'models' );
		mkdir( $folder .'/'. 'views' );

		write_file(APPPATH.'modules/'.$name.'/controllers/'.ucfirst($name).'.php',$this->_controller_creator($name, $extendsName = 'CI_Controller'));
		write_file(APPPATH.'modules/'.$name.'/models/'.ucfirst($name).'_model.php',$this->_model_creator($name, $table = NULL, $primaryKey = NULL, $extendsName = 'CI_Model'));
	}
	...
	public function hmvc_controller($hmvc_name = Null, $name = Null, $extendsName = 'CI_Controller')
	{
		// No param, Will response help.
		if ( ! isset( $name ) ) {
			echo "\n\033[33mUsage:\n\033[0m";
		...
		// Recursive create folder and return path
		$path = $this->_folder_creator($name, 'modules/'.$hmvc_name.'/controllers');

		// File exist.
		if ( file_exists(APPPATH.'modules/controllers/'.$path.'.php') ) {
			echo "This controller file already exists.\n";
			return false;
		}

		// Actually write file.
		if (!write_file(APPPATH.'modules/'.$hmvc_name.'/controllers/'.ucfirst($name).'.php',$this->_controller_creator($name, $extendsName))) {
			echo "Unable to write the file.\n";
			return false;
		}
		echo $path . " controller was created!\n";
	...
	public function hmvc_model($hmvc_name = Null, $name = Null, $table = Null, $primaryKey = Null, $extendsName = 'CI_Model')
	{
		// No param, Will response help.
		if ( ! isset( $name ) ) {
			echo "\n\033[33mUsage:\n\033[0m";
		...
		// File exist.
		if ( file_exists(APPPATH.'modules/'.$hmvc_name.'models/'.ucfirst($name).'_model.php') ) {
			echo "This model file already exists.\n";
			return false;
		}
		// Actually write file.
		if (! write_file(APPPATH.'modules/'.$hmvc_name.'/models/'.ucfirst($name).'_model.php',$this->_model_creator($name, $table, $primaryKey, $extendsName))) {
			echo "Unable to write the file.\n";
			return false;
		}
		echo $name . " model was created!\n";
		return true;
	}
